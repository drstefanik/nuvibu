...
JOB_NAME="nuvibu-ciao-che-torna-release-v2-20260820"
...
echo "Creating/updating Emma e il Ciao che Torna v2 in the production database"
gcloud run jobs deploy "${JOB_NAME}" \
  ...
  --command python \
  --args scripts/create_ciao_che_torna_episode_v2.py \
  --tasks 1 \
  --parallelism 1 \
  --max-retries 0 \
  --task-timeout 600s \
